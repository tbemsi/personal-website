---
title: "Comparison of clustering methods on Birmingham dataset"
author: "Bemsibom Toh"
date: "03/11/2020"
output:
...
birmingham.aris <- get(load('sim_20201009_ri.RData')) #ARIs from the other methods

#profile regression with no outcome
filelist = list.files("SimulationOutputNoOutcome", pattern = "\\pear_aris.txt$", full.names=TRUE) 
...
filelist = list.files("SimulationOutputRandomOutcome", pattern = "\\pear_aris.txt$", full.names=TRUE)
...
filelist = list.files("SimulationOutputDetOutcome", pattern = "\\pear_aris.txt$", full.names=TRUE)
...
filelist = list.files("DetOutcomeOne", pattern = "\\pear_aris.txt$", full.names=TRUE)
...
filelist = list.files("DetOutcomeThree", pattern = "\\pear_aris.txt$", full.names=TRUE)
...
birmingham.dfs <- get(load('sim_20201009_df.rData'))
sample.dataframe <- birmingham.dfs[[1]]
inputs <- transform_bham_data(sample.dataframe)
...
pheatmap::pheatmap(inputs$inputData[2:29],  
                   cluster_rows = F,
                   cluster_cols = F,
                   show_rownames = FALSE,
                   legend = FALSE,
...
We will use three methods to assess convergence of the profile regression algorithm: a visual method using traceplots of certain parameters, the [Gelman Diagnostic](insert_link_here), and the [Geweke Diagnostic](insert_link_here).
